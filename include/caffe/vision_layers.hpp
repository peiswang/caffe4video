...
/**
 * @brief Convolves the input image sequence with a bank of learned filters,
 *        and (optionally) adds biases.
 *
 *   Caffe convolves by reduction to matrix multiplication. This achieves
 *   high-throughput and generality of input and filter dimensions but comes at
...
class MultiConvolutionLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides MultiConvolutionParameter multi_convolution_param,
   *    with MultiConvolutionLayer options:
   *  - num_output. The number of filters.
   *  - kernel_size / kernel_h / kernel_w. The filter dimensions, given by
   *  kernel_size for square filters or kernel_h and kernel_w for rectangular
   *  filters.
   ...
   *    kernels + stream parallelism) engines.
   */
  explicit MultiConvolutionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_MULTICONVOLUTION;
  }
  virtual inline int MinBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline bool EqualNumBottomTopBlobs() const { return true; }
...
template <typename Dtype>
class RecursiveOnceLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides RecursiveOnceParameter recursive_param,
   *    with RecursiveOnceLayer options:
   *  - num_output. The number of filters.
   *  - kernel_size / kernel_h / kernel_w. The filter dimensions, given by
   *  kernel_size for square filters or kernel_h and kernel_w for rectangular
   *  filters.
   ...
   *    kernels + stream parallelism) engines.
   */
  explicit RecursiveOnceLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_RECURSIVE_ONCE;
  }
  virtual inline int MinBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline bool EqualNumBottomTopBlobs() const { return true; }
...
/**
 * @brief Pools the input video frames by taking the max, average, etc. across temporal.
 *
...
class TemporalPoolingLayer : public Layer<Dtype> {
 public:
  ...
  //virtual inline int MaxTopBlobs() const {
    //return (this->layer_param_.temporal_pooling_param().pool() ==
    //        TemporalPoolingParameter_PoolMethod_MAX) ? 2 : 1;
  //}
