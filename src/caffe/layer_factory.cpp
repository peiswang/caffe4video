...
// Get multi_convolution layer according to engine.
template <typename Dtype>
MultiConvolutionLayer<Dtype>* GetMultiConvolutionLayer(const string& name,
    const LayerParameter& param) {
  MultiConvolutionParameter_Engine engine = param.multi_convolution_param().engine();
  if (engine == MultiConvolutionParameter_Engine_DEFAULT) {
    engine = MultiConvolutionParameter_Engine_CAFFE;
#ifdef USE_CUDNN
    engine = MultiConvolutionParameter_Engine_CUDNN;
#endif
  }
  if (engine == MultiConvolutionParameter_Engine_CAFFE) {
    return new MultiConvolutionLayer<Dtype>(param);
#ifdef USE_CUDNN
  } else if (engine == MultiConvolutionParameter_Engine_CUDNN) {
    return new MultiCuDNNConvolutionLayer<Dtype>(param);
#endif
  } else {
    LOG(FATAL) << "Layer " << name << " has unknown engine.";
  }
...
template MultiConvolutionLayer<float>* GetMultiConvolutionLayer(const string& name,
    const LayerParameter& param);
...
// Get temporal_convolution layer according to engine.
template <typename Dtype>
TemporalConvolutionLayer<Dtype>* GetTemporalConvolutionLayer(const string& name,
    const LayerParameter& param) {
  TemporalConvolutionParameter_Engine engine = param.temporal_convolution_param().engine();
  if (engine == TemporalConvolutionParameter_Engine_DEFAULT) {
    engine = TemporalConvolutionParameter_Engine_CAFFE;
#ifdef USE_CUDNN
    engine = TemporalConvolutionParameter_Engine_CUDNN;
#endif
  }
  if (engine == TemporalConvolutionParameter_Engine_CAFFE) {
    return new TemporalConvolutionLayer<Dtype>(param);
#ifdef USE_CUDNN
  } else if (engine == TemporalConvolutionParameter_Engine_CUDNN) {
    return new CuDNNTemporalConvolutionLayer<Dtype>(param);
#endif
  } else {
    LOG(FATAL) << "Layer " << name << " has unknown engine.";
  }
...
template TemporalConvolutionLayer<float>* GetTemporalConvolutionLayer(const string& name,
    const LayerParameter& param);
