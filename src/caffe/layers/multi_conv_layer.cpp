...
void MultiConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  // Configure the kernel size, padding, stride, and inputs.
  MultiConvolutionParameter conv_param = this->layer_param_.multi_convolution_param();
  ...
  num_output_ = this->layer_param_.multi_convolution_param().num_output();
  CHECK_GT(num_output_, 0);
  group_ = this->layer_param_.multi_convolution_param().group();
  ...
  bias_term_ = this->layer_param_.multi_convolution_param().bias_term();
  if (this->blobs_.size() > 0) { // initialized in parent's constructor (initial param)
...
template <typename Dtype>
void MultiConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  num_ = bottom[0]->num();
  ...
  // MK * KN = MN
  M_ = num_output_ / group_;  // number of kernels 
  K_ = channels_ * kernel_h_ * kernel_w_ / group_;  // kernel size 
  N_ = height_out_ * width_out_;  // number of positions while do convolution 
...
template <typename Dtype>
void MultiConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
void MultiConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
